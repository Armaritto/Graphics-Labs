...
void main()
{
    // Simple pass-through to show the texture
...
class Model:
    def __init__(self, filepath):
        ...
        glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 8 * ctypes.sizeof(ctypes.c_float), ctypes.c_void_p(12))
        glEnableVertexAttribArray(1)

        # Normals (location = 2)
        glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, 8 * ctypes.sizeof(ctypes.c_float), ctypes.c_void_p(20))
    ...
    def center_model(self):
        ...
        scale_factor = 10.0 / max_size  # Adjust the scale for your scene
...
def main():
    ...
    window = glfw.create_window(800, 600, "OBJ Viewer with Camera Controls", None, None)
    if not window:
        glfw.terminate()
    ...
    while not glfw.window_should_close(window):
        ...
        # We won't animate the model, so keep identity for 'model'
        identity_model = Matrix44.identity()

        glUniformMatrix4fv(model_loc, 1, GL_FALSE, identity_model)
